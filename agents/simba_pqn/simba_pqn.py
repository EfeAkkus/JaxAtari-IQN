...
def single_run(config: dict):
    ...
    print(f"[simba_pqn] start compile...")
    ...
    print(f"[simba_pqn] compilation time: {end_compile - start_compile:.2f}s")
    ...
    print(f"[simba_pqn] starting training for {config.get('TOTAL_TIMESTEPS')} steps...")
    while global_step < config.get("TOTAL_TIMESTEPS"):
        ...
        print(f"[simba_pqn] iteration {iteration} | global_step {global_step} | avg_return {infos['returned_episode_returns'][-1].mean():.2f} | avg_length {infos['returned_episode_lengths'][-1].mean():.2f} | td_loss {loss[-1]:.4f} | q_val {q_val[-1]:.4f} | SPS {int(global_step / (time.perf_counter() - run_time))} | SPS_update {int(steps_per_iteration / (time.perf_counter() - iteration_time_start))}")
        metrics = {
