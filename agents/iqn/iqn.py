...
def single_run(config: dict):
    ...
    def step_once(carry, unused_step):
        ...
        def do_update(update_carry, _):
            ...
            (loss, q_val), grads = jax.value_and_grad(iqn_loss_fn, has_aux=True)(u_state.params)
            new_state = u_state.apply_gradients(grads=grads)

            return (new_state, u_key), loss

        def run_updates(s_state, s_key):
            (new_s_state, new_s_key), losses = jax.lax.scan(
                do_update, (s_state, s_key), None, length=updates_per_step
            )
